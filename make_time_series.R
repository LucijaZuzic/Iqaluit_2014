...
for (stormnum in names(case_data)) {
  ...
  if (makeplot) {
    ...
    ggsave(paste0("afullframe_time_R.pdf"), plot = p, width = 10, height = 4.5, device = cairo_pdf)
    ggsave(paste0("afullframe_time_R.png"), plot = p, width = 10, height = 4.5)
    ...
    write_csv(tffilt, "afullframe_filter_new.csv")
    
    dff <- read_csv("afullframe_filter_new.csv", show_col_types = FALSE)
    
    secticks <- c(0, 31 * 24 * 3600, 59 * 24 * 3600, 90 * 24 * 3600, 120 * 24 * 3600, 151 * 24 * 3600, 181 * 24 * 3600, 212 * 24 * 3600, 243 * 24 * 3600, 273 * 24 * 3600, 304 * 24 * 3600, 334 * 24 * 3600)
    seclabs <- c("1", "32", "60", "91", "121", "152", "182", "213", "244", "274", "305", "335")
    
    plot_title <- paste0("Time series of filtered horizontal positioning errors [m]\n(< ", filtval, ") for Iqaluit, NU, Canada (2014)")
    ...
    ggsave(paste0("afullframe_filter_R.pdf"), plot = p, width = 10, height = 4.5, device = cairo_pdf)
    ggsave(paste0("afullframe_filter_R.png"), plot = p, width = 10, height = 4.5)
  }
}